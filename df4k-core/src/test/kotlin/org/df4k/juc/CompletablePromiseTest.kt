...
import org.df4k.core.connector.messagescalar.CompletablePromise.supplyAsync
...
class CompletablePromiseTest {
    ...
    fun testExceptions() {
        ...
        val future = CompletablePromise<Any>()
        ...
        val future2 = Companion.supplyAsync { throw RuntimeException() }
        future2.exceptionally({ t ->
            t.printStackTrace()
            throw CompletionException(t)
        })

        val future3 = Companion.supplyAsync { "test" }
    ...
    fun testCancellation() {
        ...
        val other = Companion.supplyAsync { "Doomed value" }
        val future = Companion.supplyAsync {
            sleep(1000)
            "Doomed value"
        }.exceptionally({ t ->
            cancelled.set(true)
    ...
    fun testExceptionally() {
        ...
        val future = CompletablePromise<Any>().exceptionally({ t ->
            called.set(true)
            null
        })
        future.completeExceptionally(CancellationException())
    ...
    fun testCompletableFutures() {
        ...
        val future1 = Companion.supplyAsync {
            sleep(900)
            "Done2."
        }
        val future2 = Companion.supplyAsync { "Constant" }
        val future3 = Companion.supplyAsync {
            sleep(500)
            throw RuntimeException("CompletableFuture4")
        }
        val future4 = CompletablePromise<String>()
        future4.completeExceptionally(RuntimeException("CompletableFuture5"))
        val future5 = Companion.supplyAsync {
            executed.set(true)
            sleep(1000)
            "Done."
        }
        ...
        val result10 = CompletablePromise<String>()
        try {
            onFailure<String>(future3, { e -> result10.complete("Failed") }).get(0, TimeUnit.SECONDS)
            fail("Didn't timeout")
        ...
        val result3 = CompletablePromise<String>()
        future.applyToEither(future1, { v -> result3.complete("Selected: $v") })
        val result4 = CompletablePromise<String>()
        ...
        val result1 = CompletablePromise<String>()
        ...
        try {
            onFailure<Void>(future3.acceptEither(future4, { e -> }), { e -> result1.complete(e.message) }).get()
            fail("Didn't fail")
        } catch (ee: ExecutionException) {
        }
        ...
        val result11 = CompletablePromise<String>()
        try {
            onFailure<Any>(future1.thenApply({ v -> future3 }), { e -> result11.complete("Failed") }).get()
        } catch (ee: ExecutionException) {
            assertEquals("Failed", result11.get())
        }

        val result2 = CompletablePromise<String>()
        onFailure<Any>(future3.thenCompose({ v -> future1 }), { e -> result2.complete("Flat map failed: $e") })
        ...
        val result5 = CompletablePromise<String>()
        val result6 = CompletablePromise<String>()
        onFailure<Void>(future.thenAccept({ s -> result5.complete("onSuccess: $s") }),
        ...
        val result7 = CompletablePromise<String>()
        val result8 = CompletablePromise<String>()
        ensure(onFailure<Void>(future3.thenAccept({ s -> result7.complete("onSuccess: $s") }), { e -> result7.complete("onFailure: $e") }), { result8.complete("Ensured") })
        ...
        val result9 = CompletablePromise<String>()
        future.thenAccept({ v -> result9.complete("onSuccess: $v") })
        assertEquals("onSuccess: Done.", result9.get())
    }

    private fun collect(completableFutures: List<CompletablePromise<String>>): CompletablePromise<List<String>> {
        val result = CompletablePromise<List<String>>()
        ...
        if (size == 0) {
            ...
            for (completableFuture in completableFutures) {
                completableFuture.handle<String>({ s, t ->
                    if (t == null) {
                        list.add(s)
                        if (list.size == size) {
                            result.complete(list)
    ...
    @SafeVarargs
    private fun select(vararg completableFutures: CompletablePromise<String>): CompletablePromise<String> {
        val future = CompletablePromise<String>()
        for (completableFuture in completableFutures) {
            completableFuture.thenAccept(Consumer { future.complete(it) })
        }
        return future
    }
    ...
    companion object {
        ...
        private fun <T> onFailure(future: CompletablePromise<T>, call: Consumer<Throwable>): CompletablePromise<T> {
            val completableFuture = CompletablePromise<T>()
        ...
        private fun <T> ensure(future: CompletablePromise<T>, call: Runnable): CompletablePromise<T> {
            val completableFuture = CompletablePromise<T>()
            future.handle<Any>({ v, t ->
                if (t == null) {
                    call.run()
